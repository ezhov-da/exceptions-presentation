 ...
 * 2. Обработка этих исключений (в какое оборачивать и прочее) зависит от договорённости в команде
 * 3. Приложение ничего не знает о том, что получение книг может выдавать ошибку
 * <p>
 * Представим, что получени книг - это одна из критичных функций нашего приложения и от бизнеса поступает запрос:
 * В случае ошибки получения книг, нам нужно попытаться получить их из другого источника.
 * <p>
 * Что мы можем сделать с нашим кодом и реализациями?
 * <p>
 * Всё верно, в сервисе принимать несколько репозиториев и в случае ошибки
 * попробовать получить данные из другого репозитория.
 */
public class App4 {
    ...
    private static class BookService {
         ...
         * 3. Может есть и третий?
         * <p>
         * Взвесив все за и против, приняв риски я выбираю второй вариант.
         * <p>
         * Казалось бы - всё хорошо, но в данной ситуациии мы:
     ...
     * 2. В обоих реализациях может произойти ошибка
     ...
     * Данные спецификации называются «контрактами» в соответствии с концептуальной метафорой условий и
     * ответственности в гражданско-правовых договорах.
     *
     * Как завещает один из языков программирования - "Явное лучше, чем неявное"
     */
